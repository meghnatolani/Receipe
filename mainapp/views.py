...
#def profile(request):
    ...
    #try:
        ...
        #else:
            #return HttpResponse(jinja_environ.get_template('profile.html').render({"author":request.user.author, "profiler":Rider.objects.get(pk=authorid)}))
    #except:
        #return HttpResponse(jinja_environ.get_template('profile.html').render({"author":request.user.author, "profiler":request.user.author}))
    ...
    ##Check if user has enabled facebook sync:
        ...
        ##Update facebook friends if more than a week
            ...
            #except:
                #pass
            #for x in friendlist:
                #for y in Rider.objects.filter(facebook_id__contains=x['id']):
...
def post_form(request):
    ...
    if retval <> None:
        return retval
    return HttpResponse(jinja_environ.get_template('post.html').render({"author":request.user.author, 'owner':request.user.author}))
...
def post_page(request):
    ...
    postobj=Post.objects.filter(pk=request.REQUEST['key'], status__lte=1)[0]
...
def signup_do(request):
    ...
    try:
        ...
        user.save()
        entry = Rider(user=user, phone=phone, gender=gender, car_number=car_number, verified = uuid.uuid4().hex)
    ...
    if request.method == "POST":
        if "access_token" in request.REQUEST.keys():
            ...
            try:
                ...
                tempauthors = Rider.objects.filter(facebook_id__contains = fbid)
            ...
            try:
                ...
                pass
            for z in friendlist:
                for y in Rider.objects.filter(facebook_id__contains=z['id']):
                    request.user.author.fbfriends.add(y)
